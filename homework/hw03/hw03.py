...
def pingpong(n):
    ...
    def pre_to_add(m):
        if m == 2:
            return 1
        elif m % 8 == 1 or num_eights(m - 1) > 0:
            return - pre_to_add(m - 1)
        else:
            return pre_to_add(m - 1)
    if n == 1:
        return 1
    else:
        return pre_to_add(n) + pingpong(n - 1)
...
def count_coins(change):
    ...
    True
    """
    def count_coins_under_c(change_, c):
        if change_ == 0:
            return 1
        elif change_ < 0:
            return 0
            ...
            return count_coins_under_c(change_ - c, c) + count_coins_under_c(change_, next_smaller_coin(c))
    return count_coins_under_c(change, 25)
...
def beaver(f):
    "*** YOUR CODE HERE ***"
    return lambda f: (lambda x: f(x(x)))(lambda x: f(x(x)))
