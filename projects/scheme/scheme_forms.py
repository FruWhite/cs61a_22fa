...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    curr = scheme_eval(expressions.first, env, True if expressions.rest is nil else False)
    if is_scheme_false(curr) or expressions.rest is nil:
        return curr
    else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    curr = scheme_eval(expressions.first, env, True if expressions.rest is nil else False)
    if expressions.rest is nil or is_scheme_true(curr):
        return curr
    else:
