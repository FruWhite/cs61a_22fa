def flatten(s):
    ...
    for x in s:
        if type(x) != list:
            y.append(x)
        elif all([type(x) != list for i in x]):
            y += x
        else:
            y += flatten(x)
    return y
...
def closer_city(lat, lon, city_a, city_b):
    ...
    if distance(city_a, base) >= distance(city_b, base):
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        # Attention: the argument 'leaves' is just a list, not a tree!
    else:
    ...
    if is_leaf(t1):
        ...
        if len(branches(t1)) <= len(branches(t2)):
            for i in range(len(branches(t1))):
                added_branches += [add_trees(branches(t1)[i], branches(t2)[i])]
