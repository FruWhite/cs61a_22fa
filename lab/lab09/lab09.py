...
def num_trees(n):
    ...
    *   *        *       *
    ...
    if n == 1:
        ...
        for i in range(1, n):
            result += cached(num_trees)(i) * cached(num_trees)(n - i)
        return result
...
def deep_len(lnk):
    ...
    if lnk == Link.empty:
        return 0
    elif not isinstance(lnk, Link) or not isinstance(lnk.first, Link) and lnk.rest == Link.empty:
        return 1
    else:
...
def long_paths(t, n):
    ...
    if t.is_leaf():
        ...
        result = []
        for b in t.branches:
            result += [[t.label] + p for p in long_paths(b, n - 1)]
        return result
