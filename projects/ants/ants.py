...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place_searching = self.place
        for _ in range(0, self.lower_bound):
            if place_searching.entrance:
                place_searching = place_searching.entrance
        if place_searching.is_hive:
            return None
        location = self.lower_bound
        while not place_searching.bees and not place_searching.entrance.is_hive and location < self.upper_bound:
            place_searching = place_searching.entrance
            location += 1
        if place_searching.bees:
            return random_bee(place_searching.bees)
        else:
            return None
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt gives protection to our glorious home base."""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eats bees in its place when not chewing."""
...
class QueenAnt(ScubaThrower):
    ...
    def construct(cls, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        if cls == QueenAnt:
            if not gamestate.is_QueenAnt_exist:
                tmp = super().construct(gamestate)
                if tmp:
                    gamestate.is_QueenAnt_exist = True
                return tmp
            else:
                return None
    ...
    def throw_at(self, target):
        ...
        def slowed_action(gamestate):
            if target.slowed_turns == 0:
                ...
                target.action = lambda g: Bee.action(target, g)
            elif gamestate.time % 2 == 1:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not searching_place.entrance.is_hive:
            searching_place = searching_place.entrance
            dis += 1
            if searching_place.ant:
                insects_and_distances[searching_place.ant] = dis
                if searching_place.ant.is_container and searching_place.ant.ant_contained:
                    insects_and_distances[searching_place.ant.ant_contained] = dis
            if searching_place.bees:
                for bee in searching_place.bees:
                    insects_and_distances[bee] = dis
        return insects_and_distances
        # END Problem Optional 2

    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage = max(self.damage - distance * 0.25 - self.insects_shot * 0.0625, 0)
        return damage
        # END Problem Optional 2
