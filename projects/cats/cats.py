...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    paras_can_select = [s for s in paragraphs if select(s)]
    if k >= len(paras_can_select):
        return ''
    else:
        return paras_can_select[k]
    # END PROBLEM 1
    ...
    if not source_words:
        ...
        for i in range(min(len(source_words), len(typed_words))):
            if typed_words[i] == source_words[i]:
                count_correct += 1
        return count_correct / len(typed_words) * 100.0
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diff_list = [diff_function(typed_word, source, limit) for source in word_list]
        min_diff = min(diff_list)
        if min_diff > limit:
            return typed_word
        else:
            for s in word_list:
                if diff_function(typed_word, s, limit) != min_diff:
                    continue
                return s
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    def compare(i, diffs = len_diff):
        if i == min_len:
            return diffs
        elif typed[i] != source[i]:
            if diffs == limit:
                return limit + 1
            return compare(i + 1, diffs + 1)
        return compare(i + 1, diffs)
    return compare(0)
...
def time_per_word(words, times_per_player):
    ...
    "*** YOUR CODE HERE ***"
    w_len = len(words)
    times = [[p[i + 1] - p[i] for i in range(w_len)] for p in times_per_player]
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    # words = [[get_all_words(match)[j] for j in word_indices if min([get_all_times(match)[k][j] for k in player_indices]) == get_all_times(match)[i][j]] for i in player_indices]
    words = get_all_words(match)
    times = get_all_times(match)
    fastest_words = [[] for i in player_indices]  # can not write [[]] * player_indices
    for j in word_indices:
        for i in player_indices:
            if min([times[k][j] for k in player_indices]) == times[i][j]:
                fastest_words[i].append(words[j])
                break
